...
class WheelDriver(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(MasterMessage, '/master_com/ros_to_master', 10)

        self.autonomy_switch(True)

    def controller_state_received(self, msg: WheelStates):
        data = [int(x) for x in [msg.front_right.velocity * METRIC_VELOCITY_TO_MOTOR_VALUE_FACTOR, 
