...
void ServiceBasedNav2Controller::
	...
	while (!cmd_vel_service_client->wait_for_service(std::chrono::seconds(1))) {
		if (!rclcpp::ok()) {
			RCLCPP_ERROR(logger, "Interrupted while waiting for \"%s\". Exiting.", cmd_vel_service_id.c_str());
			return;
		}
...
PLUGINLIB_EXPORT_CLASS(service_based_nav2_controller::ServiceBasedNav2Controller, nav2_core::Controller)
