...
public:
  ...
  rclcpp::Client<service_based_nav2_controller_srvs::srv::ComputeVelocityCommands>::SharedPtr
    cmd_vel_service_client;
};

void ServiceBasedNav2Controller::configure(
  const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent, std::string name,
  std::shared_ptr<tf2_ros::Buffer>, std::shared_ptr<nav2_costmap_2d::Costmap2DROS>)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".compute_velocity_commands_service", rclcpp::ParameterValue(""));
  std::string cmd_vel_service_id =
    node->get_parameter(name + ".compute_velocity_commands_service").as_string();

  // Create service client for cmd_vel service.
  cmd_vel_service_client =
    node->create_client<service_based_nav2_controller_srvs::srv::ComputeVelocityCommands>(
      cmd_vel_service_id);
  ...
  while (!cmd_vel_service_client->wait_for_service(std::chrono::seconds(1))) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(
        logger, "Interrupted while waiting for \"%s\". Exiting.", cmd_vel_service_id.c_str());
      return;
    }
    RCLCPP_INFO(logger, "\"%s\" not available, waiting again...", cmd_vel_service_id.c_str());
  }
}
...
geometry_msgs::msg::TwistStamped ServiceBasedNav2Controller::computeVelocityCommands(
  const geometry_msgs::msg::PoseStamped & pose, const geometry_msgs::msg::Twist & velocity,
  nav2_core::GoalChecker *)
{
  // Create request for cmd_vel service.
  auto request =
    std::make_shared<service_based_nav2_controller_srvs::srv::ComputeVelocityCommands::Request>();
  request->pose = pose;
  ...
  if (future.wait_for(std::chrono::seconds(1)) != std::future_status::ready) {
    RCLCPP_ERROR(logger, "Failed to call \"%s\".", cmd_vel_service_client->get_service_name());
...
void ServiceBasedNav2Controller::setPlan(const nav_msgs::msg::Path & path) { this->path = path; }
...
PLUGINLIB_EXPORT_CLASS(
  service_based_nav2_controller::ServiceBasedNav2Controller, nav2_core::Controller)
