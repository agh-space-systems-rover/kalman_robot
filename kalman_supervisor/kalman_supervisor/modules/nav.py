...
class BasicNav:
    ...
    def configure(self) -> None:
        self.supervisor.declare_parameter("nav.robot_frame", "base_link")

    def activate(self) -> None:
        self.robot_frame = self.supervisor.get_parameter("nav.robot_frame").value

        self.__client = ActionClient(self.supervisor, NavigateToPose, "nav/navigate_to_pose")
        ...
        if self.__nav2_req_future is not None:
            if self.__nav2_req_future.done():
                ...
                if goal_handle.accepted:
                    ...
                    self.supervisor.get_logger().error("[Nav2] Goal rejected. This should never happen. Awaiting further goals.")
                    self.__last_goal_status = GoalStatus.STATUS_SUCCEEDED
    ...
    def send_goal(self, pos: np.ndarray, frame: str) -> None:
        ...
        pos = self.supervisor.tf.transform_numpy(pos, frame, self.supervisor.map.frame())
        ...
        if self.supervisor.map.occupancy(pos, frame) == Map.Occupancy.OUT_OF_BOUNDS:
            robot_pos = self.supervisor.tf.transform_numpy(np.zeros(3), frame, self.robot_frame)
            goal_to_robot = robot_pos - pos
            new_pos = self.supervisor.map.closest_free_in_dir(pos, frame, goal_to_robot)
            if new_pos is not None:
                pos = new_pos
                self.supervisor.get_logger().info("Goal is too far away. Traveling to a partial goal.")
        ...
        if self.supervisor.map.occupancy(pos, frame) != Map.Occupancy.FREE:
            ...
            if new_pos is not None:
                ...
                self.supervisor.get_logger().error("Goal is inside an obstacle, but there is no free space to travel to. Goal will not be sent.")
...
class NavWithRecovery:
    ...
    def activate(self) -> None:
        self.basic_nav.activate()
        self.robot_frame = self.basic_nav.robot_frame
    ...
    def tick(self) -> None:
        ...
        if self.__goal is not None and not self.basic_nav.has_goal() and self.__recovery_state == NavWithRecovery.RecoveryState.DISENGAGED:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                ...
                if self.supervisor.map.occupancy(np.zeros(3), self.robot_frame) == Map.Occupancy.FREE:
                    self.supervisor.get_logger().error("Goal has failed, but robot is not stuck. Is the goal unreachable? Will not attempt recovery.")
                    self.__goal = None
                    self.__last_goal_status = GoalStatus.STATUS_SUCCEEDED
                    return
                else:
                    self.supervisor.get_logger().info("Goal has failed and robot is stuck in costmap. Attempting recovery...")

                    # Find the nearest free position to send the robot there.
                    robot_pos = self.supervisor.tf.transform_numpy(np.zeros(3), self.__goal[1], self.robot_frame)
                    self.__recovery_free_pos = self.supervisor.map.closest_free(robot_pos, self.__goal[1]), self.__goal[1]
                    if self.__recovery_free_pos is None:
                        self.__recovery_free_pos = self.supervisor.position_history.latest_free()
                        if self.__recovery_free_pos is None:
                            self.supervisor.get_logger().error("No free cells around the robot and there's no free positions in history. Recovery is impossible.")
                            self.__recovery_state = NavWithRecovery.RecoveryState.DISENGAGED
                            self.__goal = None
                    ...
                    robot_to_free /= np.linalg.norm(robot_to_free[:2]) # The positions should be in map's frame and thus Z can be zeroed.
                    self.__recovery_free_pos = self.__recovery_free_pos[0] + robot_to_free, self.__recovery_free_pos[1]

                    # Before sending the robot to the free position, disable obstacle layers.
                    self.supervisor.map.request_to_disable_obstacle_layers()
                    self.__recovery_state = NavWithRecovery.RecoveryState.REQUESTED_TO_DISABLE_OBSTACLE_LAYERS
        
        if self.__recovery_state == NavWithRecovery.RecoveryState.REQUESTED_TO_DISABLE_OBSTACLE_LAYERS:
            if not self.supervisor.map.obstacle_layers_enabled():
                ...
                self.supervisor.get_logger().info("Sending the robot to an unobstructed position.")
                self.basic_nav.send_goal(*self.__recovery_free_pos)
                self.__recovery_state = NavWithRecovery.RecoveryState.DRIVING_TO_FREE_POS

        if self.__recovery_state == NavWithRecovery.RecoveryState.DRIVING_TO_FREE_POS:
            if not self.basic_nav.has_goal():
                self.supervisor.get_logger().info("Robot is in position. Re-enabling obstacle layers...")
                self.supervisor.map.request_to_enable_obstacle_layers()
                self.__recovery_state = NavWithRecovery.RecoveryState.REQUESTED_TO_REENABLE_OBSTACLE_LAYERS

        if self.__recovery_state == NavWithRecovery.RecoveryState.REQUESTED_TO_REENABLE_OBSTACLE_LAYERS:
            if self.supervisor.map.obstacle_layers_enabled():
                # Recovery is complete - send the original goal, rinse and repeat.
                self.supervisor.get_logger().info("Recovery is complete. Sending the original goal again.")
                self.__recovery_state = NavWithRecovery.RecoveryState.DISENGAGED
                # No other goals should be running at the time of recovery.
                self.send_goal(*self.__goal)

        if self.__recovery_state == NavWithRecovery.RecoveryState.ABORTING_RECOVERY_WAIT_FOR_LAYERS_AND_GOAL:
            if self.supervisor.map.obstacle_layers_enabled() and not self.basic_nav.has_goal():
                self.__recovery_state = NavWithRecovery.RecoveryState.DISENGAGED
                self.__goal = None
                self.__last_goal_status = GoalStatus.STATUS_CANCELED
                self.supervisor.get_logger().info("Recovery has been aborted. Goal has been canceled.")

    def deactivate(self) -> None:
        self.basic_nav.deactivate()

    def has_goal(self) -> bool:
        return self.__goal is not None or self.__recovery_state != NavWithRecovery.RecoveryState.DISENGAGED
    ...
    def cancel_goal(self) -> None:
        if self.__recovery_state != NavWithRecovery.RecoveryState.DISENGAGED:
            ...
            self.basic_nav.cancel_goal()
            self.supervisor.map.request_to_enable_obstacle_layers()
            self.__recovery_state = NavWithRecovery.RecoveryState.ABORTING_RECOVERY_WAIT_FOR_LAYERS_AND_GOAL
...
class NavWithQueuedGoals:
    ...
    def activate(self) -> None:
        self.nav_w_recovery.activate()
        self.robot_frame = self.nav_w_recovery.robot_frame
...
class NavWithLongGoals:
    ...
    def activate(self) -> None:
        self.nav_w_queue.activate()
        self.robot_frame = self.nav_w_queue.robot_frame

        self.__goal: tuple[np.ndarray, str] | None = None
        self.__goal_is_long = False
        self.__last_pos_on_success: np.ndarray | None = None
        self.__last_goal_status = GoalStatus.STATUS_SUCCEEDED
    ...
    def tick(self) -> None:
        ...
        if self.__goal is not None and self.nav_w_queue.has_goal() and self.__goal_is_long:
            if self.supervisor.map.occupancy(self.__goal[0], self.__goal[1]) != Map.Occupancy.OUT_OF_BOUNDS:
                self.supervisor.get_logger().info("Long goal just entered the map. Navigating directly there.")
        ...
        if self.__goal is not None and not self.nav_w_queue.has_goal():
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                if self.__goal_is_long:
                    ...
                    robot_pos = self.supervisor.tf.transform_numpy(np.zeros(3), self.__goal[1], self.robot_frame)
                    if self.__last_pos_on_success is None or np.linalg.norm(robot_pos - self.__last_pos_on_success) > 2.0:
                        self.supervisor.get_logger().info("Partial goal reached. Navigating further to the final goal.")
                        self.nav_w_queue.send_goal(*self.__goal)
                        self.__last_pos_on_success = robot_pos
                        return
            
            self.__goal = None
            self.__goal_is_long = False
            self.__last_pos_on_success = None
