...
class BasicNav:
    ...
    def send_goal(self, pos: np.ndarray, frame: str) -> None:
        ...
        pos = self.supervisor.tf.transform_numpy(pos, self.supervisor.map.frame(), frame)
    ...
    def tick(self) -> None:
        ...
        if self.__goal is not None and not self.basic_nav.has_goal() and self.__recovery_state == NavWithRecovery.RecoveryState.DISENGAGED:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                ...
                if self.supervisor.map.occupancy(np.zeros(3), self.supervisor.tf.robot_frame()) == Map.Occupancy.FREE:
...
class NavWithLongGoals:
    ...
    def tick(self) -> None:
        ...
        if self.__goal is not None and self.__goal_is_long and self.nav_w_queue.has_goal():
        ...
        if self.__goal is not None and self.__goal_is_long and self.nav_w_queue.has_goal():
            last_corrected_goal = self.nav_w_queue.nav_w_recovery.basic_nav.last_corrected_goal()
            if last_corrected_goal is not None:
                ...
                if np.linalg.norm(robot_pos - goal_pos) < 3.0 and (self.__last_goal_start_pos is None or np.linalg.norm(robot_pos - self.__last_goal_start_pos) > 2.0):
                    self.supervisor.get_logger().info("Almost at the partial goal. Navigating further to the final goal.")
                    self.nav_w_queue.send_goal(*self.__goal)
        ...
        if self.__goal is not None and not self.nav_w_queue.has_goal():
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                if self.__goal_is_long:
                    ...
                    if self.__last_goal_start_pos is None or np.linalg.norm(robot_pos - self.__last_goal_start_pos) > 2.0:
                        self.supervisor.get_logger().info("Partial goal reached. Navigating further to the final goal.")
                        self.nav_w_queue.send_goal(*self.__goal)
                        self.__last_goal_start_pos = robot_pos
                        return
                    else:
                        self.supervisor.get_logger().info("Failed to advance towards a long goal. Nevertheless, goal is considered successful.")
    ...
    def distance_to_goal(self) -> float:
        if self.__goal is not None:
            robot_pos = self.supervisor.tf.robot_pos()[:2]
            goal_pos = self.supervisor.tf.transform_numpy(self.__goal[0], self.supervisor.tf.world_frame(), self.__goal[1])[:2]
