...
class Yolo(Module):
    ...
    def activate(self) -> None:
        self.__get_state_client = self.supervisor.create_client(GetState, f"yolo/get_state")
        self.__change_state_client = self.supervisor.create_client(ChangeState, f"yolo/change_state")
        self.__detection_sub = self.supervisor.create_subscription(Detection2DArray, f"yolo/detections", self.__detection_callback, 10)
    ...
    def tick(self) -> None:
        ...
        if self.__enabled != self.__should_be_enabled and self.__change_state_future is None and self.__get_state_future is None:
        ...
        if self.__get_state_future is not None and self.__get_state_future.done():
            ...
            if response.current_state.id == State.PRIMARY_STATE_INACTIVE and self.__should_be_enabled:
                ...
                self.__change_state_future = self.__change_state_client.call_async(request)
        ...
        if self.__change_state_future is not None and self.__change_state_future.done():
            ...
            self.__detections.clear()
            self.supervisor.get_logger().info(f"[YOLO] Detection is now {'enabled' if self.__enabled else 'disabled'}.")
    ...
    def __detection_callback(self, msg: Detection2DArray) -> None:
        max_detection_distance = self.supervisor.get_parameter("yolo.max_detection_distance").value
