...
class ArUco(Module):
    ...
    def activate(self) -> None:
        ...
        for i in range(num_cameras):
            self.__get_state_clients.append(self.supervisor.create_client(GetState, f"aruco/get_state{i}"))
            self.__change_state_clients.append(self.supervisor.create_client(ChangeState, f"aruco/change_state{i}"))
            self.__detection_subs.append(self.supervisor.create_subscription(
    ...
    def tick(self) -> None:
        ...
        if self.__enabled != self.__should_be_enabled and len(self.__change_state_futures) == 0 and len(self.__get_state_futures) == 0:
            # Wait for get state services to be ready.
            for i, client in enumerate(self.__get_state_clients):
                if not client.service_is_ready():
                    self.supervisor.get_logger().info(f"Waiting for aruco/get_state{i}...")
                    client.wait_for_service()

            request = GetState.Request()
            self.__get_state_futures = [client.call_async(request) for client in self.__get_state_clients]

        # If a get state request is pending and it is done, optionally follow with a state change request.
        if len(self.__get_state_futures) != 0 and all([future.done() for future in self.__get_state_futures]):
            ...
            for i, client in enumerate(self.__change_state_clients):
                if not client.service_is_ready():
                    self.supervisor.get_logger().info(f"Waiting for aruco/change_state{i}...")
                    client.wait_for_service()

            # Process the responses and send state change requests if needed.
            for response, change_state_client in zip(responses, self.__change_state_clients):
                if response.current_state.id == State.PRIMARY_STATE_INACTIVE and self.__should_be_enabled:
                    request = ChangeState.Request()
                    request.transition.id = Transition.TRANSITION_ACTIVATE
                    self.__change_state_futures.append(change_state_client.call_async(request))
                elif response.current_state.id == State.PRIMARY_STATE_ACTIVE and not self.__should_be_enabled:
                    request = ChangeState.Request()
        ...
        if len(self.__change_state_futures) != 0 and all([future.done() for future in self.__change_state_futures]):
            self.__change_state_futures.clear()

            self.__enabled = not self.__enabled
            self.__detections.clear()
            self.supervisor.get_logger().info(f"[ArUco] Detection is now {'enabled' if self.__enabled else 'disabled'}.")
    ...
    def __detection_callback(self, msg: ArucoDetection) -> None:
        max_detection_distance = self.supervisor.get_parameter("aruco.max_detection_distance").value

        frame = msg.header.frame_id

        # This should not happen, but just in case.
        if not self.supervisor.tf.can_transform(self.supervisor.tf.world_frame(), frame) or not self.supervisor.tf.can_transform(self.supervisor.tf.robot_frame(), frame):
            return
        ...
        for marker_msg in msg.markers:
            ...
            world_pos = self.supervisor.tf.transform_numpy(pos, self.supervisor.tf.world_frame(), frame)
            ...
            if id not in self.__detections:
                self.supervisor.get_logger().info(f"[ArUco] Detected marker #{id} in frame {frame}.")
