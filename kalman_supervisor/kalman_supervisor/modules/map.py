...
class Map(Module):
    ...
    def activate(self) -> None:
        self.obstacle_layers = self.supervisor.get_parameter("map.obstacle_layers").value
        ...
        self.__global_param_set_client = self.supervisor.create_client(SetParameters, "map/global/set_parameters")
        self.__global_param_set_future: Future | None = None
        self.__local_param_set_client = self.supervisor.create_client(SetParameters, "map/local/set_parameters")
    ...
    def __map_update_cb(self, msg: OccupancyGridUpdate) -> None:
        ...
        if x0 < 0 or y0 < 0 or x1 > self.__data_np.shape[1] or y1 > self.__data_np.shape[0]:
            return
