...
MAX_DISTANCE = 20.0
CEILING_STOP_BUFFER = 2.0  # stop 2 m after lost ceiling
ARUCO_STOP_MIN_DISTANCE_TRAVELED = 10000.0  # infty = disabled
...
class RscpLavatubeExplore(State):
    ...
    def enter(self) -> None:
        ...
        self.fetch_default_follower_approach_distance()
        self.supervisor.get_logger().info(
            "[RSCP] Entering lava tube via tunnel follower"
        )
    ...
    def tick(self) -> str | None:
        ...
        if traveled > ARUCO_STOP_MIN_DISTANCE_TRAVELED:
            if (
                self.supervisor.aruco.marker_pos(
                    GATE_MARKER_ID, WORKING_FRAME, ARUCO_STOP_DETECTION_TIMEOUT
                )
                is not None
            ):
                self.supervisor.get_logger().info(
                    "[RSCP] Aruco marker detected, stopping"
                )
                return self.finish(traveled)
        ...
        if (
            ...
            self.supervisor.get_logger().info(
                "[RSCP] Lost ceiling for too long, stopping"
            )
            return self.finish(traveled)
