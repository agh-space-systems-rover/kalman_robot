...
class ArduinoDriving(Node):
    def __init__(self):
        ...
        self.max_translation_angle = self.declare_parameter(
            "max_translation_angle", 110 * np.pi / 180
        )
    ...
    def joy_cb(self, msg: Joy):
        ...
        rotate_in_place = (
            msg.buttons[LEFT_STICK_BUTTON] or msg.buttons[RIGHT_STICK_BUTTON]
        )
        if rotate_in_place:
            ...
            drive.inv_radius = msg.axes[LEFT_X] / lerp(
                self.max_turn_radius.value, self.min_turn_radius.value, max_turn
            )
        ...
        if self.drive_msg_is_zero(drive) and self.drive_msg_is_zero(
            self.last_drive_msg
        ):
            return
