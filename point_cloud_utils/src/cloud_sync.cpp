...
{
  using type = typename approx_policy_n<N - 1, sensor_msgs::msg::PointCloud2, Args...>::type;
};
...
{
  using type = typename exact_policy_n<N - 1, sensor_msgs::msg::PointCloud2, Args...>::type;
};
...
public:
  ...
  std::shared_ptr<rclcpp::Subscription<sensor_msgs::msg::PointCloud2>> subscriber;
  ...
  CloudSync(const rclcpp::NodeOptions & options) : Node("cloud_sync", options)
  {
    // Declare parameters.
    declare_parameter("number_of_inputs", 0);
    declare_parameter("approx_sync", true);
    declare_parameter("queue_size", 10);
    declare_parameter("output_frame_id", "base_link");
    declare_parameter("fixed_frame_id", "odom");
    ...
    point_cloud_publisher = create_publisher<sensor_msgs::msg::PointCloud2>("output", 10);
    ...
    switch (number_of_inputs) {
      ...
      default:
        throw std::runtime_error(
          "Invalid number of input topics: " + std::to_string(number_of_inputs));
    }
  }

  template <typename... Args>
  bool concat_clouds(const sensor_msgs::msg::PointCloud2::SharedPtr & in, Args... other_in)
  {
    ...
    try {
      transform = tf_buffer->lookupTransform(
        output_cloud->header.frame_id, output_cloud->header.stamp, in->header.frame_id,
        in->header.stamp, fixed_frame_id, rclcpp::Duration::from_seconds(1));
    } catch (...) {
      return false;
    }

    // Transform the point cloud to the output frame.
    pcl_ros::transformPointCloud(output_cloud->header.frame_id, transform, *in, *transformed_cloud);
    ...
    pcl::concatenatePointCloud(*output_cloud, *transformed_cloud, *output_cloud);
  ...
  struct sync_callback_n
  {
    using type = typename sync_callback_n<
      N - 1, const sensor_msgs::msg::PointCloud2::SharedPtr &, Args...>::type;
  };
  ...
  std::shared_ptr<message_filters::Synchronizer<SyncPolicy>> create_synchronizer(Args &&... args)
  {
    if constexpr (N == 0) {
  ...
  void run_with_sync_policy()
  {
    ...
    synchronizer_typed->registerCallback(&sync_callback_type::sync_callback, &sync_callback);
  ...
  {
    // Create the callback.
    using sync_callback_type = sync_callback_t<sensor_msgs::msg::PointCloud2::SharedPtr>;
    auto sync_callback = sync_callback_type(this);
    sync_callback_keepalive = sync_callback;
    ...
    subscriber = create_subscription<sensor_msgs::msg::PointCloud2>(
      "input", queue_size,
      std::bind(&sync_callback_type::sync_callback, sync_callback, std::placeholders::_1));
  }
};
