...
public:
  ...
  ObstacleDetection(const rclcpp::NodeOptions & options) : Node("obstacle_detection", options)
  {
    ...
    sub = create_subscription<sensor_msgs::msg::PointCloud2>(
      "input", queue_size, std::bind(&ObstacleDetection::callback, this, std::placeholders::_1));
  }
  ...
  void callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    try {
      pc_to_world_tf =
        tf_buffer->lookupTransform(world_frame, msg->header.frame_id, tf2::TimePointZero);
    ...
    if (cloud->size() > 0) {
      ...
      try {
        robot_to_world_tf =
          tf_buffer->lookupTransform(world_frame, robot_frame, tf2::TimePointZero);
      } catch (tf2::TransformException & ex) {
        RCLCPP_ERROR(get_logger(), "%s", ex.what());
        return;
      }
      ...
      for (size_t i = 0; i < cloud->size(); i++) {
        ...
        if (
          position.z() > robot_z + min_obstacle_height &&
