...
public:
	...
	ObstacleDetection(const rclcpp::NodeOptions &options) : Node("obstacle_detection", options) {
		// Declare parameters.
		declare_parameter("queue_size", 10);
		declare_parameter("world_frame", "odom");
		declare_parameter("normal_estimation_radius", 0.2);
		declare_parameter("max_ground_angle", 25 * M_PI / 180);
		declare_parameter("outlier_removal_radius", 0.2);
		declare_parameter("outlier_removal_min_neighbors", 10);
		declare_parameter("robot_frame", "base_link");
		declare_parameter("min_obstacle_height", -1.0);
		declare_parameter("max_obstacle_height", 1.0);
		...
		// Create a subscriber.
		sub = create_subscription<sensor_msgs::msg::PointCloud2>("input", queue_size, std::bind(&ObstacleDetection::callback, this, std::placeholders::_1));
	}
	...
	void callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
		...
		get_parameter("outlier_removal_min_neighbors", outlier_removal_min_neighbors);
		...
		try {
			pc_to_world_tf = tf_buffer->lookupTransform(world_frame, msg->header.frame_id, tf2::TimePointZero);
		...
		if (cloud->size() > 0) {
			...
			try {
				robot_to_world_tf = tf_buffer->lookupTransform(world_frame, robot_frame, tf2::TimePointZero);
			} catch (tf2::TransformException &ex) {
				RCLCPP_ERROR(get_logger(), "%s", ex.what());
				return;
			}
			Eigen::Isometry3d robot_to_world = tf2::transformToEigen(robot_to_world_tf);
			...
			for (size_t i = 0; i < cloud->size(); i++) {
				...
				Eigen::Vector3d position(cloud->points[i].x, cloud->points[i].y, cloud->points[i].z);
				position = pc_to_world * position;

				if (position.z() > robot_z + min_obstacle_height && position.z() < robot_z + max_obstacle_height) {
