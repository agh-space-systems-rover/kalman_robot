...
public:
	GripperRepublisher() : Node("gripper_republisher") {
		...
		gripper_pub_ = this->create_publisher<std_msgs::msg::Int8>(
		    "/gripper/command_incremental", 10
		);

		fk_sub_ =
		    this->create_subscription<kalman_interfaces::msg::ArmFkCommand>(
		        "/station/arm/fk/command",
		        10,
		...
		spacenav_sub_ = this->create_subscription<sensor_msgs::msg::Joy>(
		    "/spacenav/joy", 10, [this](sensor_msgs::msg::Joy::SharedPtr msg) {
			    spacenav_callback(msg);
		    }
		);
	}
...
private:
	...
	void spacenav_callback(const sensor_msgs::msg::Joy::SharedPtr msg) {
		auto now = rclcpp::Clock().now();
		if (now - spacenav_last_time_ >
		    rclcpp::Duration::from_seconds(1.0 / rate_)) {
	...
	bool all_zeros(const kalman_interfaces::msg::ArmFkCommand::SharedPtr msg) {
		return (
		    msg->joint_1 == 0.0 && msg->joint_2 == 0.0 && msg->joint_3 == 0.0 &&
		    msg->joint_4 == 0.0 && msg->joint_5 == 0.0 && msg->joint_6 == 0.0
		);
	}
	...
	rclcpp::Subscription<kalman_interfaces::msg::ArmFkCommand>::SharedPtr
	                                                       fk_sub_;
