...
CANMapping::CANMapping(
    ...
    : can_id(can_id), ros_topic(ros_topic), ros_type(ros_type),
      max_rate(max_rate), conversion_type(conversion_type),
      extended_id(can_id > 0x7FF) {}

CANBridge::CANBridge(const rclcpp::NodeOptions &options)
    : Node("configurable_can_bridge", options), can_socket_(-1) {
	...
	can_receive_timer_ = this->create_wall_timer(
	    10ms, std::bind(&CANBridge::receiveCANMessages, this)
	);
...
bool CANBridge::loadConfig() {
	...
	try {
		...
		if (config["rx_mappings"]) {
			for (const auto &mapping : config["rx_mappings"]) {
				...
				if (mapping["conversion"] && mapping["conversion"]["type"]) {
					conversion_type =
					    mapping["conversion"]["type"].as<std::string>();
				}

				rx_mappings_.emplace_back(
				    can_id, ros_topic, ros_type, max_rate, conversion_type
				);
			}
		}
		...
		RCLCPP_INFO(
		    get_logger(),
		    "Loaded %ld RX mappings and %ld TX mappings",
		    rx_mappings_.size(),
		    tx_mappings_.size()
		);
...
bool CANBridge::initializeCANSocket() {
	...
	if (can_socket_ < 0) {
		RCLCPP_ERROR(
		    get_logger(), "Error opening CAN socket: %s", strerror(errno)
		);
		return false;
	}
	...
	if (ioctl(can_socket_, SIOCGIFINDEX, &ifr) < 0) {
		RCLCPP_ERROR(
		    get_logger(),
		    "Error getting interface index for %s: %s",
	...
	if (bind(can_socket_, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		RCLCPP_ERROR(
		    get_logger(), "Error binding CAN socket: %s", strerror(errno)
		);
		close(can_socket_);
		can_socket_ = -1;
		return false;
	...
	tv.tv_usec = 1;
	setsockopt(
	    can_socket_, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof tv
	);

	RCLCPP_INFO(
	    get_logger(),
	    "CAN socket initialized successfully on %s",
	    can_interface_.c_str()
	);
	return true;
}
...
void CANBridge::processRxMapping(CANMapping &mapping) {
	...
	if (publisher) {
		...
		RCLCPP_INFO(
		    get_logger(),
		    "Created publisher for CAN ID 0x%X -> %s",
		    mapping.can_id,
		    mapping.ros_topic.c_str()
		);
	}
}
...
void CANBridge::processTxMapping(CANMapping &mapping) {
	...
	if (subscriber) {
		...
		RCLCPP_INFO(
		    get_logger(),
		    "Created subscriber for %s -> CAN ID 0x%X",
		    mapping.ros_topic.c_str(),
		    mapping.can_id
		);
	}
...
CANBridge::createPublisher(const CANMapping &mapping) {
	try {
		return this->create_generic_publisher(
		    mapping.ros_topic, mapping.ros_type, rclcpp::QoS(10)
		);
	} catch (const std::exception &e) {
		RCLCPP_ERROR(
		    get_logger(),
		    "Failed to create publisher for topic %s with type %s: %s",
		    mapping.ros_topic.c_str(),
		    mapping.ros_type.c_str(),
		    e.what()
		);
		return nullptr;
	}
...
CANBridge::createSubscriber(CANMapping *mapping) {
	try {
		auto callback = [this, mapping](
			...
			if (mapping->ros_type == "std_msgs/msg/Int32") {
				...
				RCLCPP_WARN(
				    get_logger(),
				    "Unsupported ROS type for CAN conversion: %s",
				    mapping->ros_type.c_str()
				);
			}
		};
		...
		return this->create_generic_subscription(
		    mapping->ros_topic, mapping->ros_type, rclcpp::QoS(10), callback
		);
	} catch (const std::exception &e) {
		RCLCPP_ERROR(
		    get_logger(),
		    "Failed to create subscriber for topic %s with type %s: %s",
		    mapping->ros_topic.c_str(),
		    mapping->ros_type.c_str(),
		    e.what()
		);
...
void CANBridge::receiveCANMessages() {
	...
	while (true) {
		...
		if (nbytes < 0) {
			...
			RCLCPP_ERROR(
			    get_logger(),
			    "Error reading from CAN socket: %s",
			    strerror(errno)
			);
			break;
		}
		...
		// {
		//   RCLCPP_WARN(get_logger(), "Incomplete CAN frame received: %ld
		//   bytes, expecting %ld", nbytes,
...
bool CANBridge::sendCANMessage(const canfd_frame &frame) {
	...
	if (nbytes != sizeof(struct canfd_frame)) {
		RCLCPP_ERROR(
		    get_logger(), "Error sending CAN frame: %s", strerror(errno)
		);
		return false;
	}
...
void CANBridge::handleCANtoROS(const canfd_frame &frame) {
	...
	//   if (mapping)
	//   {
	//     RCLCPP_INFO(get_logger(), "Received CAN message for %s",
	//     mapping->ros_topic.c_str());
	//   }
	...
	if (converter_it != can_to_ros_converters_.end()) {
		if (now() - mapping->last_sent_stamp <
		    rclcpp::Duration::from_seconds(1.0 / mapping->max_rate)) {
			return;
		}
		...
		RCLCPP_WARN(
		    get_logger(),
		    "No converter available for ROS type: %s",
		    mapping->ros_type.c_str()
		);
	}
}
