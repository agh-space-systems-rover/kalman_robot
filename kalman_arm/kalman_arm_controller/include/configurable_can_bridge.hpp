...
private:
	...
	std::unordered_map<uint32_t, rclcpp::GenericPublisher::SharedPtr>
	    publishers_;
	...
	createPublisher(const CANMapping &mapping);
	rclcpp::GenericSubscription::SharedPtr
	createSubscriber(CANMapping *mapping);
	...
	template <typename T> void registerConverter(const std::string &name);
