...
int main(int argc, char **argv) {
	...
	// or a URDF and SRDF. This is, however, not the recommended way to
	// instantiate a PlanningScene. The
	...
	//   robot_model_loader(planning_scene_tutorial_node, "robot_description");
	//   const moveit::core::RobotModelPtr& kinematic_model =
	//   robot_model_loader.getModel(); planning_scene::PlanningScene
	//   planning_scene(kinematic_model);

	std::string robot_description_name = "robot_description";
	// node->get_parameter_or("robot_description_name", robot_description_name,
	// robot_description_name);

	auto planning_scene_monitor_ =
	    std::make_shared<planning_scene_monitor::PlanningSceneMonitor>(
	        planning_scene_tutorial_node,
	        robot_description_name,
	...
	// self_collision_request_.group_name = "kalman_arm";
	...
	while (true) {
		...
		scene_collision_request_.detailed_distance = true;
		scene_collision_request_.verbose           = true;
		scene_collision_request_.contacts          = true;
		scene_collision_request_.max_contacts      = 3;
		// scene_collision_request_.group_name = "kalman_arm";
		...
		self_collision_request_.max_contacts      = 3;
		// Check collision with environment.
		scene_collision_result_.clear();
		locked_scene->getCollisionEnv()->checkRobotCollision(
		    scene_collision_request_,
		    scene_collision_result_,
		    robot_state_,
		...
		RCLCPP_INFO_STREAM(
		    ...
		    "Test 1: Current state is "
		        << (scene_collision_result_.collision ? "in" : "not in")
		        << " scene collision"
		);

		auto distances = scene_collision_result_.distance_result.distances;
		for (const auto &[key, value] : distances) {
			RCLCPP_INFO_STREAM(
			    LOGGER,
			    "Distance between: " << key.first << " and " << key.second
			                         << ":\t" << value[0].distance
			);
		}
		scene_collision_result_.print();

		// Check robot self collision.
		self_collision_result_.clear();
		locked_scene->getCollisionEnvUnpadded()->checkSelfCollision(
		    self_collision_request_,
		    self_collision_result_,
		    robot_state_,
		    locked_scene->getAllowedCollisionMatrix()
		);

		self_collision_result_.print();

		RCLCPP_INFO_STREAM(
		    LOGGER,
		    "Test 2: Current state is "
		        << (self_collision_result_.collision ? "in" : "not in")
		        << " self collision"
		);

		RCLCPP_INFO_STREAM(
		    LOGGER,
		    "Test 2: Closest distance is " << self_collision_result_.distance
		);
		distances = self_collision_result_.distance_result.distances;
		auto min_distance =
		    self_collision_result_.distance_result.minimum_distance;

		RCLCPP_INFO_STREAM(
		    LOGGER,
		    "Closest are " << min_distance.link_names[0] << " and "
		                   << min_distance.link_names[1]
		...
		// rclcpp:spin(planning_scene_tutorial_node);
	}
	// Collision Checking
	// ^^^^^^^^^^^^^^^^^^
	//
	...
	//   RCLCPP_INFO_STREAM(LOGGER, "Test 1: Current state is " <<
	...
	//   planning_scene.checkSelfCollision(collision_request, collision_result);
	//   RCLCPP_INFO_STREAM(LOGGER, "Test 2: Current state is " <<
	//   (collision_result.collision ? "in" : "not in")
	//                                                          << " self
	//                                                          collision");
	...
	//   collision_result.clear();
	//   planning_scene.checkSelfCollision(collision_request, collision_result);
	//   RCLCPP_INFO_STREAM(LOGGER, "Test 3: Current state is " <<
	//   (collision_result.collision ? "in" : "not in")
	//                                                          << " self
	//                                                          collision");
	...
	//   collision_result.clear();
	//   planning_scene.checkSelfCollision(collision_request, collision_result);
	//   RCLCPP_INFO_STREAM(LOGGER, "Test 5: Current state is " <<
	//   (collision_result.collision ? "in" : "not in")
	//                                                          << " self
	//                                                          collision");
	//   collision_detection::CollisionResult::ContactMap::const_iterator it;
	//   for (it = collision_result.contacts.begin(); it !=
	//   collision_result.contacts.end(); ++it)
	//   {
	//     RCLCPP_INFO(LOGGER, "Contact between: %s and %s",
	...
	//   planning_scene.getAllowedCollisionMatrix(); moveit::core::RobotState
	...
	//   collision_result.clear();
	//   planning_scene.checkSelfCollision(collision_request, collision_result,
	//   copied_state, acm); RCLCPP_INFO_STREAM(LOGGER, "Test 6: Current state
	//   is " << (collision_result.collision ? "in" : "not in")
	//                                                          << " self
	//                                                          collision");
	...
	//   copied_state, acm); RCLCPP_INFO_STREAM(LOGGER, "Test 7: Current state
	//   is " << (collision_result.collision ? "in" : "not in")
	//                                                          << " self
	//                                                          collision");
	...
	//   // set: i.e. JointConstraint, PositionConstraint, OrientationConstraint
	//   and
	...
	//   // on the end-effector of the panda_arm group of the Panda robot. Note
	//   the
	...
	//   copied_state.update();
	//   bool constrained = planning_scene.isStateConstrained(copied_state,
	//   goal_constraint); RCLCPP_INFO_STREAM(LOGGER, "Test 8: Random state is "
	...
	//   kinematic_constraint_set(kinematic_model);
	//   kinematic_constraint_set.add(goal_constraint,
	//   planning_scene.getTransforms()); bool constrained_2 =
	//   planning_scene.isStateConstrained(copied_state,
	//   kinematic_constraint_set); RCLCPP_INFO_STREAM(LOGGER, "Test 9: Random
	//   state is " << (constrained_2 ? "constrained" : "not constrained"));

	//   // There's a direct way to do this using the KinematicConstraintSet
	//   // class.

	//   kinematic_constraints::ConstraintEvaluationResult
	//   constraint_eval_result =
