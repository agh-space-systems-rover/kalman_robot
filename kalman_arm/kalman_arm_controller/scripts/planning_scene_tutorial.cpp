...
//   planning_scene::PlanningScene planning_scene(kinematic_model);
    ...
    auto planning_scene_monitor_ = std::make_shared<planning_scene_monitor::PlanningSceneMonitor>(
        planning_scene_tutorial_node, robot_description_name, "monitored_planning_scene");

    // planning_scene_monitor_->requestPlanningSceneState();
    planning_scene_monitor_->startSceneMonitor("servo_node/publish_planning_scene");

    // self_collision_request_.group_name = "kalman_arm";

    while(true){

    auto robot_state_ = planning_scene_monitor_->getPlanningScene()->getCurrentState();
    ...
    RCLCPP_INFO_STREAM(LOGGER, "Test 1: Current state is " << (scene_collision_result_.collision ? "in" : "not in")
                                                         << " scene collision");

    auto distances = scene_collision_result_.distance_result.distances;
    for (const auto& [key, value] : distances){
        RCLCPP_INFO_STREAM(LOGGER, "Distance between: " << key.first << " and " << key.second
                                    << ":\t" << value[0].distance);
    }
    ...
    RCLCPP_INFO_STREAM(LOGGER, "Test 2: Current state is " << (self_collision_result_.collision ? "in" : "not in")
                                                         << " self collision");


    RCLCPP_INFO_STREAM(LOGGER, "Test 2: Closest distance is " << self_collision_result_.distance);
    distances = self_collision_result_.distance_result.distances;
    auto min_distance = self_collision_result_.distance_result.minimum_distance;

    RCLCPP_INFO_STREAM(LOGGER, "Closest are " << min_distance.link_names[0] <<" and " << min_distance.link_names[1] <<" \tdistance: "
