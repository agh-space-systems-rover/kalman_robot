...
class MasterToServo : public rclcpp::Node {
  public:
	MasterToServo(const rclcpp::NodeOptions &options)
		...
		// Setup pub/sub
		joy_sub_ =
		    this->create_subscription<kalman_interfaces::msg::ArmCompressed>(
		        JOY_TOPIC,
		        rclcpp::SystemDefaultsQoS(),
		        [this](
		            const kalman_interfaces::msg::ArmCompressed::ConstSharedPtr
		                &msg
		        ) {
			        return joyCB(msg);
		        }
		...
		spacemouse_sub_ =
		    this->create_subscription<kalman_interfaces::msg::MasterMessage>(
		        ...
		        rclcpp::SystemDefaultsQoS(),
		        [this](
		            const kalman_interfaces::msg::MasterMessage::ConstSharedPtr
		                &msg
		        ) {
			        return spacemouseCB(msg);
		        }
		    );
		...
		pose_abort_pub_ =
		    this->create_publisher<example_interfaces::msg::Empty>(
	...
	void joyCB(kalman_interfaces::msg::ArmCompressed::ConstSharedPtr msg) {
		...
		for (uint8_t i = 0; i < 6; i++) {
			joint_msg->joint_names.push_back(
			    "arm_joint_" + std::to_string(i + 1)
			);
	...
	void
	spacemouseCB(kalman_interfaces::msg::MasterMessage::ConstSharedPtr msg) {
		control_type_pub_->publish(std_msgs::msg::UInt8().set__data(1));
		trajectory_abort_pub_->publish(
		    std::make_unique<example_interfaces::msg::Empty>()
		);
  ...
  private:
	...
	rclcpp::Publisher<example_interfaces::msg::Empty>::SharedPtr
	    pose_abort_pub_;
	rclcpp::Publisher<example_interfaces::msg::Empty>::SharedPtr
	                                                  trajectory_abort_pub_;
