 ...
 */
enum positioningStatus_t : uint8_t {
 ...
 */
struct [[gnu::packed]] jointMotorStatus_t {
	union {
		struct [[gnu::packed]] {
			uint8_t motorTemperatureFault      : 1;
			uint8_t controllerTemperatureFault : 1;
 ...
 */
struct [[gnu::packed]] jointMotorFastStatus_t {
	int16_t velocity; // RPM*10
	int32_t position; // pozycja 0-36000 (co 0.01 deg)
};
 ...
 */
struct [[gnu::packed]] jointCmdSetpoint_t {
 ...
 */
struct [[gnu::packed]] jointCmdVelocity_t {
 ...
 */
struct [[gnu::packed]] jointCmdControlType_t {
