...
/**
 * @brief Update joint setpoint from global joints - converts from human
 * readable units to CAN message types. Also calculates differential joint
 * setpoints.
 */
...
/**
 * @brief Update joint setpoint from global joints depending on differential
 * joint
 *
...
/**
 * @brief Update joint setpoint from global joints - converting to human
 * readable units
 *
 * @param joint_id - from 0 to 5
 */
...
/**
 * @brief Update joint setpoint from global joints - converting to CAN message
 * types
 *
 * @param joint_id - from 0 to 5
 */
...
void CAN_vars::calculate_diff_setpoints() {
	for (uint8_t i = 0; i < arm_config.jointNumber; i++) {
		const jointConfig_t *config = &CAN_vars::arm_config.joint[i + 1];
 ...
 */
void CAN_vars::calculate_single_diff_setpoint(
    uint8_t joint_id, uint8_t diff_id
) {
