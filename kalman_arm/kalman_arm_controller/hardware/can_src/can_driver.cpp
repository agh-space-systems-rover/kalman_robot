 ...
 * @return int 0 on success, 1 on failure
 */
int CAN_driver::handle_frame(canfd_frame frame, const std::unordered_map<uint8_t, canCmdHandler_t>* handles)
{
  // Decode the frame
...
int CAN_driver::write_fastclick(DriverVars_t* driver_vars, uint8_t position)
{
  ...
  return write_data(driver_vars, can_id, (uint8_t*)&data, LEN_CMD_SET_FASTCLICK);
}
}
