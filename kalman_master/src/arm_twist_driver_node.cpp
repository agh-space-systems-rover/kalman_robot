...
class ArmTwistDriver : public rclcpp::Node {
  public:
	ArmTwistDriver(const rclcpp::NodeOptions &options)
		...
		publisher_ =
		    this->create_publisher<kalman_interfaces::msg::MasterMessage>(
		...
		axes_locks_sub_ =
		    this->create_subscription<kalman_interfaces::msg::ArmAxesLocks>(
		        "arm/axes_locks",
		        10,
		        [this](kalman_interfaces::msg::ArmAxesLocks::SharedPtr msg) {
			        axes_locks_ = msg;
		        }
		    );
	}
  ...
  private:
	...
	void republish_master_msg(const geometry_msgs::msg::Twist::SharedPtr msg) {
		if (send_) {
			auto master_msg =
			    std::make_shared<kalman_interfaces::msg::MasterMessage>();
			master_msg->cmd =
	...
	bool all_zeros(const geometry_msgs::msg::Twist::SharedPtr msg) {
		return (
		    ...
		    msg->linear.z == 0.0 && msg->angular.x == 0.0 &&
	...
	rclcpp::Subscription<kalman_interfaces::msg::ArmAxesLocks>::SharedPtr
	    axes_locks_sub_;
