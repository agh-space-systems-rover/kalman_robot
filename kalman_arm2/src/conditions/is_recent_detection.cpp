...
IsRecentDetection::IsRecentDetection(
    ...
    : BT::SimpleConditionNode(
          name, std::bind(&IsRecentDetection::check, this), config
      ),
      parent_(parent) {
	...
	aruco_sub_ = parent_->create_subscription<ArucoDetection>(
	    ...
	    10,                           // queue size
	    std::bind(
	        &IsRecentDetection::aruco_callback, this, std::placeholders::_1
	    )
	);
}
...
void IsRecentDetection::aruco_callback(
	...
	// FIXME: store this detection, in check() just compare the header and
	// current time
...
BT::NodeStatus IsRecentDetection::check(void) {
	...
	if (!max_age_msec_optional) {
		RCLCPP_ERROR_STREAM(
		    parent_->get_logger(),
		    name() << ": max_age_ms not set, check file: "
		); // TODO: Add a filename here
		return BT::NodeStatus::FAILURE;
	}

	const float max_age_msec = max_age_msec_optional.value();
	const auto  max_age_seconds =
	    rclcpp::Duration::from_seconds(max_age_msec / 1000.0F);
...
IsRecentDetection::get_first_aruco_position() const {
