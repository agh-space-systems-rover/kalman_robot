...
class PoseIK : public rclcpp::Node {
  public:
    ...
    PoseIK(const rclcpp::NodeOptions &options)
        ...
        joint_vel_pub =
            create_publisher<kalman_interfaces::msg::ArmValues>("target_vel", 10);
        ...
        compute_timer = create_wall_timer(
            timer_period,
            std::bind(&PoseIK::compute_joint_velocities, this)
        );
    ...
    Eigen::VectorXd vector_from_parameter(
        ...
        if (values.size() != size) {
            RCLCPP_WARN(
                get_logger(),
                "Parameter '%s' has %zu elements, expected %zu. Using defaults.",
                name.c_str(),
                values.size(),
                size
            );
    ...
    void on_robot_description(const std_msgs::msg::String::SharedPtr msg) {
            ...
            if (auto urdf_joint = model.getJoint(joint.getName())) {
                if (urdf_joint->limits) {
                    joint_preferred_positions(joint_index) =
                        0.5 * (urdf_joint->limits->lower + urdf_joint->limits->upper);
                }
            }
            ++joint_index;
        }
        ...
        RCLCPP_INFO(
            get_logger(),
            "PoseIK initialized with %d joints. motion_weights=%s avoidance_gains=%s",
    ...
    Eigen::VectorXd compute_posture_bias(const Eigen::VectorXd &q) const {
        Eigen::VectorXd bias = joint_centering_gain * (joint_preferred_positions - q);
        for (Eigen::Index i = 0; i < q.size(); ++i) {
            ...
            if (std::abs(joint_angle) < threshold) {
                ...
                const double ratio =
                    1.0 - (std::abs(joint_angle) / threshold);
    ...
    bool solve_joint_velocities(
        ...
        twist_vector << base_twist.linear.x,
            base_twist.linear.y,
            base_twist.linear.z,
        ...
        const Eigen::MatrixXd weighted_pseudoinverse =
            weight_inv * jacobian.transpose() * task_metric.ldlt().solve(
    ...
    void compute_joint_velocities() {
        ...
        try {
            ...
            const geometry_msgs::msg::Twist base_twist =
                pose_error_to_twist(current_pose, target_pose);
