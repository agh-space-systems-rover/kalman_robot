...
MAX_DISTANCE_RAD = 0.35  # about 20 deg
...
try:
    with open(f"{arm_config}/config/predefined_poses.yaml", "r") as f:
        ...
        for pose in predefined_poses["poses"]:
            PREDEFINED_POSES[pose["name"]] = f"{arm_config}/{pose['path']}"
except:
...
class PoseRequestSender(Node):
    ...
    def __init__(self):
        ...
        for name, value in PREDEFINED_POSES.items():
            self.execute_subs[name] = self.create_subscription(
                Empty,
                f"/go_to_{name}/execute",
                lambda msg, path=value: self.send_goal(path),
                10,
            )

        self._status_pub = self.create_publisher(UInt8, f"/pose_request/status", 10)
