...
class TunnelFollower(Node):
    def __init__(self):
        ...
        self.viz_pub = self.create_publisher(
            MarkerArray, "/arc/tunnel_debug_markers", 10
        )
        self.cloud_pub = self.create_publisher(
            PointCloud2, "/arc/tunnel_debug_cloud", 10
        )
        ...
        self.pc_sub = self.create_subscription(
            PointCloud2, "/d455_front/point_cloud", self.cloud_cb, 1
        )

        self.srv = self.create_service(SetBool, "/arc/follow_tunnel", self.toggle_cb)
    ...
    def cloud_cb(self, msg):
        ...
        mask = (
            (pts_all[:, 2] > self.z_min)
            & (pts_all[:, 2] < self.z_max)
            & (pts_all[:, 0] > 0.1)
            & (pts_all[:, 0] < self.x_max)
