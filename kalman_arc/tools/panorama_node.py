...
class PanoramaApp:
    ...
    def setup_ui(self):
        ...
        self.status_label = tk.Label(
            self.root, text="Status: Ready", font=("Arial", 14, "bold"), fg="blue"
        )
        self.status_label.pack(pady=5)

        self.info_label = tk.Label(
            self.root, text="Yaw: 0° | Progress: 0/360°", font=("Arial", 12)
        )
        ...
        tk.Button(
            btn_frame, text="↺ Reset", command=self.reset_recording, bg="gold", width=10
        ).pack(side=tk.LEFT, padx=5)
    ...
    def process_recording(self, yaw, raw_bytes):
        ...
        if abs(yaw - self.last_saved_yaw) % 360 >= self.angle_threshold:
            filename = os.path.join(
                self.current_frames_dir, f"frame_{self.saved_count:03d}.jpg"
            )
            ...
            self.info_label.configure(
                text=f"Yaw: {yaw:.1f}° | Progress: {self.accumulated_angle:.0f}/360° | Saved: {self.saved_count}"
            )
    ...
    def process_stitching(self):
        ...
        if status == cv2.Stitcher_OK:
            path = os.path.join(
                self.panoramas_dir,
                f"panorama_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}.jpg",
            )
            ...
            self.root.after(
                ...
                lambda: messagebox.showerror(
                    "Error", f"Stitching failed with code: {status}"
                ),
            )
