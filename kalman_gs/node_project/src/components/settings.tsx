...
import { faBan, faCloudMoon, faKeyboard, faPalette, faRefresh, faSun } from '@fortawesome/free-solid-svg-icons';
...
export default class Settings extends Component<{}, State> {
  ...
  render() {
    const searchedKeybinds = Object.entries(keybinds).map(([action, key]) => (
      ...
      <div className={styles['keybind']} key={action}>
        ...
        <div className={styles['keybind-controls']}>
          ...
          <Button tooltip='Reset this keybind.' onClick={() => {
    ...
    return (
      <div
        className={
          styles['settings-bg'] + (this.state.shown ? ` ${styles['shown']}` : '')
        }
        onClick={() => this.hide()}
      >
        <div
