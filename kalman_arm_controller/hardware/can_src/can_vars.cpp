...
{
    const jointConfig_t* config = &CAN_vars::arm_config.joint[joint_id + 1];
    const jointMotorFastStatus_t* jointStatus = &CAN_vars::joints[joint_id].fastStatus;
...
void CAN_vars::update_single_joint_setpoint(uint8_t joint_id)
{
    ...
    temp = (((10.0f * 60) / 360.0f) / gearRatio) * joints[joint_id].moveSetpoint.velocity_deg_s *
           direction;
    if (temp >= INT16_MAX)
